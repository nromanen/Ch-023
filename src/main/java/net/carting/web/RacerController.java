...
public class RacerController {
	...
	public String racerPage(Map<String, Object> map, @PathVariable("id") int id) {
		...
		racer = racerService.getRacerById(id);
		String username = userService.getCurrentUserName();
		String authority = userService.getCurrentAuthority();
		String permission = "access denied";
		boolean accessGranted = false;
		if (authority.equals(ROLE_TEAM_LEADER)) accessGranted = true;
		map.put("accessGranted", accessGranted);
		...
		if (accessGranted) {
			...
			racer.setDocument(permission);
			racer.setAddress(permission);
			map.put("racer", racer);
		}
	...
	public String editRacerPage(Map<String, Object> map,
		...
		if (userService.getCurrentAuthority().equals(ROLE_TEAM_LEADER)) {
			...
			if (team.getId() == racer.getTeam().getId()) {
				...
				map.put("dateOfBirth",
						formatter.get().format(racer.getBirthday()));
	...
	public @ResponseBody int editRacerAction(
			@RequestBody Map<String, Object> racerMap) {
		Racer racer = new Racer();
		...
		racer.setBirthday(DateUtil.getDateFromString(racerMap.get("birthday")
				.toString()));
	...
	public @ResponseBody int addRacerAction(
			@RequestBody Map<String, Object> racerMap) {
		...
		String racerCarClassNumbersStr = racerMap.get("carClassNumbers")
				.toString();
		Set<RacerCarClassNumber> racerCarClassNumbers = racerService
				.parseCarClasses(carClassesStr, racerCarClassNumbersStr, racer);
		// ------------------------------------------------------------
		...
		while (it.hasNext()) {
			racerCarClassNumberService
					.addRacerCarClassNumber((RacerCarClassNumber) it.next());
		}
		// --------------------------------------
	...
	public @ResponseBody boolean isSetNumberForCarClass(
	...
	@RequestMapping(value = "/getOccupiedDefaultNumbersForCarClass", method = RequestMethod.POST, headers = { "content-type=application/json" })
	public @ResponseBody String getOccupiedDefaultNumbersForCarClassAction(
			@RequestBody Map<String, Object> map) {
		int carClassId = Integer.parseInt(map.get("carClassId").toString());
		try {
			return racerCarClassNumberService.getNumbersByCarClassId(
					carClassId, ",");
		} catch (Exception e) {
			return "error";
		}
	...
	public @ResponseBody boolean isSetCarClass(
			@RequestBody Map<String, Object> map) {
		int carClassId = Integer.parseInt(map.get("carClassId").toString());
	...
	public @ResponseBody String addRacerCarClassNumber(
			@RequestBody Map<String, Object> map) {
		int racerId = Integer.parseInt(map.get("racerId").toString());
		...
		String username = userService.getCurrentUserName();
		LOG.info(username + " has added car class" + carClass.getName()
				+ "» with #" + number + " to racer " + racer.getFirstName()
				+ " " + racer.getLastName() + "(id = " + racerId + ")");

		return "success";
	}

	@RequestMapping(value = "/updateRacerCarClassNumbers", method = RequestMethod.POST, headers = { "content-type=application/json" })
	public @ResponseBody String updateRacerCarClassNumbers(
			@RequestBody Map<String, Object> map) {
		int racerId = Integer.parseInt(map.get("racerId").toString());
		Racer racer = racerService.getRacerById(racerId);
		...
		Set<RacerCarClassNumber> racerCarClassNumbers = racerService
				.parseUpdatedRacerCarClassNumbers(
	...
	public @ResponseBody String deleteRacerCarClassNumbers(
			@RequestBody Map<String, Object> map) {
		int racerId = Integer.parseInt(map.get("racerId").toString());
		Racer racer = racerService.getRacerById(racerId);
		for (RacerCarClassNumber racerCarClassNumber : racer
			...
			List<CarClassCompetition> carClassCompetitions = carClassCompetitionService
					.getCarClassCompetitionsByCarClassId(carClassId);
			for (CarClassCompetition carClassCompetition : carClassCompetitions) {
				racerCarClassCompetitionNumberService
						.deleteByCarClassCompetitionIdAndRacerId(
								carClassCompetition.getId(), racerId);
			}
			racerCarClassNumberService
					.deleteRacerCarClassNumber(racerCarClassNumber);
		}
	...
	public @ResponseBody String deleteChosenRacerCarClassNumbers(
			@RequestBody Map<String, Object> map) {
	...
	public @ResponseBody boolean isSetRacer(@RequestBody Map<String, Object> map) {
		String firstName = map.get("firstName").toString();
		String lastName = map.get("lastName").toString();
		Date birthday = DateUtil.getDateFromString(map.get("birthday")
				.toString());
		return racerService.isSetRacer(firstName, lastName, birthday);
	}

	@RequestMapping(value = "/delete", method = RequestMethod.POST, headers = { "content-type=application/json" })
	public @ResponseBody String deleteRacer(@RequestBody Map<String, Object> map) {
		Racer racer = racerService.getRacerById(Integer.parseInt(map.get("id")
				.toString()));
		racerService.deleteRacer(racer);

		String username = userService.getCurrentUserName();
		LOG.info(username + " has deleted racer " + racer.getFirstName() + " "
				+ racer.getLastName() + "(id = " + racer.getId() + ")");

		return "success";
	}

	@RequestMapping(value = "/setEnabled", method = RequestMethod.POST, headers = { "content-type=application/json" })
	public @ResponseBody String setRacerEnabled(
			@RequestBody Map<String, Object> map) {
		Racer racer = racerService.getRacerById(Integer.parseInt(map.get(
				"racerId").toString()));
		...
		String username = userService.getCurrentUserName();
		LOG.info(username + " has " + (enabled ? "enabled" : "disabled")
				+ " racer " + racer.getFirstName() + " " + racer.getLastName()
				+ "(id = " + racer.getId() + ")");
