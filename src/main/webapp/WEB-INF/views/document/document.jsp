...
<div class="panel panel-primary">
		...
		<table class="width-100">
			<tr>
				<td><span class="glyphicon glyphicon-list-alt"></span> <label
					class="text-info" style="color: #fff; font-size: 16px;"> <c:choose>
				...
				<td><c:if test="${document.teamOwner==team}">
						<a href="#" class="btn btn-danger" id="delete_document_btn"
							style="float: right"><spring:message code="label.delete" /></a>
						<a href="<c:url value="/document/edit/${document.id}" />"
							class="btn btn-info" id="edit_document_btn" style="float: right"><spring:message
								code="label.edit" /></a>
					</c:if></td>
			</tr>
		</table>
	...
	<div class="panel-body">
		<table style="float: right">
			<tr>
				<c:if test="${authority=='ROLE_ADMIN'}">
					<td><div class="btn-group" data-toggle="buttons">
							...
							<input type="hidden" id="approved_url"
			...
			<tr>
				<td><c:if test="${document.approved}">
					...
					</c:if> <c:if test="${document.approved==false && document.checked}">
		...
		<c:if
			test="${document.type==1 || document.type == 3 || document.type==4 || document.type==5 }">
			<div>
		...
		<c:if test="${document.reason != null && !document.reason.equals('')}">
			<div>
				...
				<textarea style="width: 50%" rows=3 class="form-control"
					id="result_sequance" disabled>${document.reason}</textarea>
			</div>
		</c:if>
...
<c:if test="${request.getParameter('competition_id') != null}">
	<a
		href="/Carting/competition/${request.getParameter('competition_id')}/mandat"
		class="btn btn-primary"><spring:message
			code="label.document_return_to_mandat_statement" /></a>
</c:if>
<div class="modal fade" id="delete_document_modal" tabindex="-1"
	role="dialog">
	<div class="modal-dialog" style="margin-top: 10%;">
		<div class="modal-content">
			...
			<div class="modal-body">
				<label class="text-info"><spring:message
						code="message.question_delete_document" /></label>
				<c:forEach items="${document.racers}" var="racer">
					<input type="hidden" class="racer_id" value="${racer.id}">
				</c:forEach>
				<input type="hidden" id="document_delete_url"
					value="<c:url value="/document/delete" />"> <input
					type="hidden" id="document_type" value="${document.type}">
				<input type="hidden" id="document_delete_id" value="${document.id}">
				<input type="hidden" id="team_id"
					value="${document.getTeamOwner().id}"> <br> <br>
				...
				<div class="form-group">
					<form data-toggle="validator">
						...
						<textarea rows=5 id="reason" class="form-control"
							placeholder="<spring:message code="placeholder.reason" />"
							id="result_sequance" required pattern=".{1,255}"
